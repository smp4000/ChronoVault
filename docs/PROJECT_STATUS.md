...
- Foto-Werkzeuge im Panel: `->imageEditor()` (Zuschneiden/Drehen/Spiegeln, Seitenverhältnisse) auf Slot-Uploads + „Weitere Fotos"; „Weitere Fotos" per Drag&Drop sortierbar (Reihenfolge gilt im Shop); Hauptbild-Konvention: Slot „Vorderseite" führt die Galerie an (`photoUrls()` sortiert Front-Slot zuerst); `WatermarkWatchPhotosAction` (GD + DejaVuSans-Bold aus dompdf, Text halbtransparent unten rechts, idempotent via custom_property watermarked, Ursprungsformat bleibt) + Header-Aktion „Wasserzeichen" auf der Edit-Seite (Text editierbar, Default Betriebsname)
