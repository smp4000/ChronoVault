...
## Zentraler Marktplatz (chrono-save.de, eBay-Prinzip: privat + gewerblich)

- Listings-Spiegel: zentrale Tabelle `marketplace_listings` (Migration 2026_07_12_100000, zentrale DB) — denormalisierte Anzeige-Daten je veröffentlichter KAUFBARER Uhr (Lager/Kommission; Verkauft/Reserviert/Service/Wunschliste/Sammlung fliegen raus — der Händler-Shop zeigt Verkauftes weiter als Referenz); Felder: tenant_id+watch_id (unique), seller_name/seller_type (commercial|private, aus Tenant-data `seller_type`, Default commercial), shop_url/detail_url (Verkäufer-Domain; https in Produktion), brand/model/reference, year/condition/material/diameter-Labels, has_box/has_papers, price/previous_price/discount_percent, photo_url (Host-Teil wird durch Verkäufer-Domain ersetzt — bewusst KEIN URL::forceRootUrl, das verstellt die Root-URL des Prozesses!), listed_at
- `App\Models\MarketplaceListing` (CentralConnection-Trait — schreibbar aus dem Tenant-Kontext heraus) + `App\Actions\Marketplace\SyncWatchToMarketplaceAction` (execute: updateOrCreate/delete, try/catch — Sync blockiert nie das Speichern; pruneTenant fürs Aufräumen)
- Sync-Wege: `WatchObserver` saved/deleted/restored (live) + Command `marketplace:sync` (Backfill/Reparatur über alle Mandanten, entfernt auch Listings gelöschter Mandanten; Schedule täglich 00:30 NACH der Wertermittlung)
- Öffentliche Seite: `MarketplaceController::index` auf den ZENTRALEN Domains (Route `/` = `marketplace.index`, ersetzt die welcome-View) — Freitextsuche (Marke/Modell/Referenz, mehrwortfähig), Markenfilter-Pills, Verkäufertyp-Filter (Alle/Gewerblich/Privat), Sortierung, Pagination 24; Views `marketplace/layout` + `index` in der Shop-Design-Sprache mit Plattform-Branding, Trust-Leiste und Verkäufer-Badge (Privat=amber/Gewerblich=blau) auf jeder Kachel; Kacheln verlinken via detail_url in den Shop des Verkäufers (Kauf/Anfrage/Preisvorschlag laufen dort)
- Tests: `MarketplaceTest` (3) — Observer-Spiegelung inkl. Entfernen bei Verkauf, zentrale Seite mit Badge/Suche/Filter, marketplace:sync-Backfill
- SELBST-REGISTRIERUNG (Etappe 2, „Jetzt verkaufen"): zentrale Routen GET/POST `/verkaufen` (POST throttle:5,60 — jede Registrierung provisioniert eine DB; ohne Routen-Namen wegen route:cache) → `SellerRegistrationController` (create/store) + `SellerRegistrationRequest` (seller_type private|commercial, shop_name, Wunsch-Slug optional [regex, RESERVED_SLUGS-Liste, Kollision gegen Tenant::withTrashed], owner_name/email/password confirmed min:10, Rechenfrage, DSGVO-Checkbox; deutsche Meldungen); Provisionierung via `CreateTenantAction` (neuer Param sellerType → Tenant-data `seller_type`, Default commercial); Views `marketplace/register` (Typ-Auswahl als Radio-Karten, Wunsch-Adresse mit Suffix-Vorschau, Hinweis Rechtspflichten für Gewerbliche) + `marketplace/registered` (Erfolgsseite: Panel-URL, Shop-URL, Rechtliches-Hinweis bei commercial); Layout: „Jetzt verkaufen"-Button im Header + Footer-Link; Tests `SellerRegistrationTest` (2: kompletter Privat-Flow inkl. „Privat"-Badge auf dem Marktplatz; vergebene/reservierte Slugs + falsches Captcha)
- SERVER-VORAUSSETZUNG für echte Kunden-Subdomains: Wildcard-DNS `*.chrono-save.de` (Cloudflare A-Record auf Server-IP, proxied) + CloudPanel-Site muss `*.chrono-save.de` als Alias bedienen — Registrierung funktioniert sonst, aber die neue Subdomain ist nicht erreichbar
- ZENTRALE ANGEBOTSSEITE (eBay-Prinzip, „Sammelstelle"): Privatverkäufer haben KEINEN öffentlichen Shop — ihre Listings verlinken auf `/angebot/{listing}` (zentrale Domain): Galerie (photo_urls-Spiegel, JS-Wechsel), Kenndaten-Chips, Beschreibung, Privatverkaufs-Hinweisbox (keine Gewährleistung/kein Widerruf), Verkäufer-Zeile mit Badge; ANFRAGE (`POST /angebot/{id}/anfrage`) und PREISVORSCHLAG (`POST /angebot/{id}/preisvorschlag`, PriceProposalRequest) laufen zentral und werden via `Tenant->run()` in die Verkäufer-DB durchgereicht (WatchInquiryMail an TenantNotifications; PriceProposal persistiert → kompletter Panel-Verhandlungs-Workflow inkl. Gegenangebot funktioniert für Private!); Registrierung privat OHNE Geschäftsname/Wunsch-URL (Formular blendet Block per JS aus, shop_name required_if:commercial, Tenant-Name = eigener Name)
- SOFORTKAUF JE UHR: `watches.allow_direct_buy` (Tenant-Migration 130000, Toggle „Sofortkauf erlauben" im Shop-Tab) → Spiegel-Feld `marketplace_listings.direct_buy` (Privat zusätzlich NUR mit hinterlegter bank_iban — Käufer überweist direkt); Migration 120000 ergänzt description/photo_urls/direct_buy am Spiegel; `centralBaseUrl()` aus tenancy.central_domains (KEIN env() zur Laufzeit — config:cache!)
- NÄCHSTE ETAPPEN: Privat-SOFORTKAUF-Abwicklung (zentraler Kauf-Flow mit Privat-Kaufvertrag OHNE Rechnung/Steuerausweis, Zahlung direkt an Verkäufer-IBAN — Button auf der Angebotsseite zeigt bis dahin einen Hinweis); ③ Kommissions-Flow/Abrechnung + Preisgestaltung; Willkommens-Mail; E-Mail-Verifizierung
...
- DESIGN-REFRESH (Chrono24-Stil): Header mit prominenter Freitext-Suchleiste (Desktop mittig, Mobile eigene Zeile) — `?suche=` sucht über Modell, Referenz UND Markenname, mehrwortfähig (alle Begriffe müssen passen, `orWhereHas('brand')`); darunter Trust-Leiste (Geprüft & dokumentiert · Sichere Zahlung · 14 Tage Widerruf · Persönliche Beratung); Listing mit Suchkontext-Kopf statt Hero bei aktiver Suche (Treffer-Zähler + „Suche zurücksetzen"), suchbewusster Empty-State; dichteres Grid (2 Spalten mobil bis 4 Spalten auf xl); Kacheln kompakter mit Ausstattungs-Chips (Zustand, Box, Papiere) und fett hervorgehobenem Preis; `$search` als optionale Layout-Variable (`?? ''`) für alle Shop-Seiten
- Detailseite: Vertrauens-Box unter dem Kaufknopf (geprüfte Uhr, Rechnung+Kaufvertrag per Mail, versicherter Versand, 14 Tage Widerruf); „Weitere Uhren" auf 4er-Grid (Controller-Limit 4)
- PANEL-REFRESH (/app): eigenes Filament-Theme `resources/css/filament/app/theme.css` (Vite-Input + `->viteTheme()`; importiert das Filament-Standard-Theme, @source auf App-Filament/Livewire/Mobile-Views): größere Rundungen auf Sections/Tabellen/Tabs, Sidebar-Trennlinie, Topbar-Blur, blaue Akzentkante am aktiven Navigationspunkt, sanfte Button/Input-Transitions; `brandName`/`brandLogo` = blauer Punkt + Betriebsname (tenant('name'), Fallback ChronoVault — gleiche Identität wie Shop/PDFs); globale Suche per Strg/Cmd+K; Navigationsgruppen fest sortiert (Bestand, Verkauf, Stammdaten [zugeklappt], Verwaltung [zugeklappt], Einstellungen)
